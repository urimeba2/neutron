...
class IptablesMeteringDriver(abstract_driver.MeteringAbstractDriver):
    ...
    def _process_associate_metering_label(self, router):
        ...
        LOG.debug("Trying _process_associate_metering_label inside IPTABLES_DRIVER with rm: {rm}".format(
            rm=rm.__dict__
        ))
    ...
    def retrieve_and_account_traffic_counters_legacy(self, router,
        ...
        LOG.debug("All routers: {routers}. The router: {router}, with RM: {rm}, has metering labels: {metering_labels}".format(
